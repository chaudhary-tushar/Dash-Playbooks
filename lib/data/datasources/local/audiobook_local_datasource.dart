...
import 'metadata_extraction_datasource.dart';
...
class AudiobookLocalDatasource {
  ...
  Future<void> saveAudiobooks(List<Audiobook> audiobooks) async {
    try {
      final audiobookModels = audiobooks.map((a) => AudiobookModel.fromDomain(a)).toList();
      
      await _isar.writeTxn(() async {
        await _isar.audiobookModels.putAll(audiobookModels);
      });
    } catch (e) {
      throw DatabaseException('Failed to save audiobooks: ${e.toString()}');
    }
  }
  ...
  Future<List<Audiobook>> getAudiobooks() async {
    try {
      ...
      throw DatabaseException('Failed to retrieve audiobooks: ${e.toString()}');
    }
  }
  ...
  Future<Audiobook?> getAudiobookById(String id) async {
    try {
      ...
      throw DatabaseException('Failed to retrieve audiobook by ID: ${e.toString()}');
    }
  }
  ...
  Future<void> savePlaybackSession(PlaybackSession session) async {
    try {
      ...
      throw DatabaseException('Failed to save playback session: ${e.toString()}');
    }
  }
  ...
  Future<PlaybackSession?> getPlaybackSession(String audiobookId) async {
    try {
      ...
      throw DatabaseException('Failed to retrieve playback session: ${e.toString()}');
    }
  }
  ...
  Future<List<Audiobook>> searchAudiobooks(String query) async {
    try {
      ...
      throw DatabaseException('Failed to search audiobooks: ${e.toString()}');
    }
  }
  ...
  Future<List<Audiobook>> filterAudiobooks(AudiobookFilter filter) async {
    try {
      ...
      throw DatabaseException('Failed to filter audiobooks: ${e.toString()}');
    }
  }
  ...
  Future<List<Audiobook>> findAudiobooks({String? author, bool? completed, int? limit}) async {
    try {
      ...
      throw DatabaseException('Failed to find audiobooks: ${e.toString()}');
    }
  }
  ...
  Future<bool> isFileAccessible(String filePath) async {
    return await _metadataExtractor.isFileAccessible(filePath);
  }
  ...
  Future<List<Audiobook>> scanDirectory(String directoryPath) async {
    try {
      ...
      if (e is FileSystemException || e is StorageException) {
        ...
        throw FileSystemException('Error scanning directory: ${e.toString()}');
      }
    }
  }
