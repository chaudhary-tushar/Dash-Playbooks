...
import '../datasources/local/audiobook_local_datasource.dart';
import '../datasources/remote/firebase_sync_datasource.dart';
...
class AudiobookRepositoryImpl implements AudiobookRepository {
  ...
  Future<void> updateAudiobook(Audiobook audiobook) async {
    try {
      ...
      final updatedAudiobooks = allAudiobooks.map((a) => a.id == audiobook.id ? audiobook : a).toList();
  ...
  Future<void> updatePlaybackPosition(String audiobookId, Duration position) async {
    try {
  ...
  Future<void> savePlaybackSession(String audiobookId, Duration position, {double? playbackSpeed}) async {
    try {
