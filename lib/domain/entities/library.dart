  ...
  factory Library.fromMap(Map<String, dynamic> map) {
    return Library(
      ...
      audiobooks: List<Audiobook>.from(
          (map['audiobooks'] as List<dynamic>).map<Audiobook>((x) => Audiobook.fromMap(x as Map<String, dynamic>))),
      lastScanAt: DateTime.fromMillisecondsSinceEpoch(map['lastScanAt']?.toInt() ?? DateTime.now().millisecondsSinceEpoch),
