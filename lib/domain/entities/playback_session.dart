...
class PlaybackSession {
  ...
  factory PlaybackSession.fromMap(Map<String, dynamic> map) {
    return PlaybackSession(
      ...
      currentPosition: Duration(milliseconds: map['currentPosition']?.toInt() ?? 0),
      playbackSpeed: map['playbackSpeed']?.toDouble() ?? 1.0,
      isPlaying: map['isPlaying'] ?? false,
      lastPlayedAt: DateTime.fromMillisecondsSinceEpoch(map['lastPlayedAt']?.toInt() ?? DateTime.now().millisecondsSinceEpoch),
  ...
  factory PlaybackSession.fromJson(String source) => PlaybackSession.fromMap(json.decode(source) as Map<String, dynamic>);
