      ...
      if (syncResult.hasConflicts) {
        ...
        if (resolutionResult.hasUnresolvedConflicts) {
          return _createSyncResult(
            ...
            message: 'Sync completed with unresolved conflicts. ${resolutionResult.resolvedItems.length} conflicts resolved, ${resolutionResult.unresolvedConflicts.length} remain.',
      ...
      switch (conflict.type) {
        ...
        case ConflictType.progress_update:
          ...
          if (localTimestamp.isAfter(remoteTimestamp)) {
            ...
            await _userRepository.updateRemoteProgress(conflict.itemId, conflict.localData.lastPosition);
            return _createResolvedConflict(
              itemId: conflict.itemId,
              resolution: ResolutionType.localWins,
              resolvedData: conflict.localData,
            ...
            await _userRepository.updateLocalProgress(conflict.itemId, conflict.remoteData.lastPosition);
            return _createResolvedConflict(
              itemId: conflict.itemId,
              resolution: ResolutionType.remoteWins,
              resolvedData: conflict.remoteData,
...
class ConflictResolutionFailedException implements Exception {
  ...
  @override
  String toString() => 'ConflictResolutionFailedException: Conflict $conflictId failed because: $reason';
}